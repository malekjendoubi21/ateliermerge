...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
   ...
   @Query("SELECT COUNT(r) FROM Reservation r " +
           "WHERE r.dateAnneeUniv " +
           "BETWEEN :debutAnnee AND :finAnnee")
    ...
    @Query("SELECT r FROM Reservation r " +
            "WHERE r.finAnneeUnive " +
            "BETWEEN :currentDate AND :expirationDate")
    ...
    @Modifying
    @Query("UPDATE Reservation r SET r.estValide = false " +
            "WHERE r.finAnneeUnive < :currentDate")
 ...
 @Query("SELECT c.typeChambre," +
         " SUM(CASE WHEN r.estValide = true THEN 1 ELSE 0 END), " +
         "SUM(CASE WHEN r.estValide = false THEN 1 ELSE 0 END) " +
         "FROM Reservation r " +
         "JOIN r.chambre c " +
         "WHERE r.dateAnneeUniv BETWEEN :startDate " +
         "AND :endDate " +
